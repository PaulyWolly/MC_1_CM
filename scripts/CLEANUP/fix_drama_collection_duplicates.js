/*
  FIX_DRAMA_COLLECTION_DUPLICATES.JS
  Version: 2.0
...
console.log('🔧 [DRAMA COLLECTION CLEANUP] Starting to fix duplicates and format issues in Drama collection...');

const collectionsPath = path.join(__dirname, '../../public/components/MediaLibrary/data/collections.json');

try {
  // Read the collections data
  console.log('📖 [DRAMA COLLECTION CLEANUP] Reading collections.json...');
  ...
  // Process Drama collection
  if (collectionsData.collections && collectionsData.collections.genres && collectionsData.collections.genres.Drama) {
    const dramaCollection = collectionsData.collections.genres.Drama;
    
    console.log(`📊 [DRAMA COLLECTION CLEANUP] Found ${dramaCollection.length} items in Drama collection`);
    
    // Track seen items to identify duplicates
    const seenItems = new Set();
    const cleanedCollection = [];
    
    dramaCollection.forEach((item, index) => {
    ...
    // Update the Drama collection
    collectionsData.collections.genres.Drama = cleanedCollection;
    
    console.log(`✅ [DRAMA COLLECTION CLEANUP] Processing complete:`);
    console.log(`   - Fixed items: ${fixedCount}`);
    console.log(`   - Final count: ${cleanedCollection.length}`);
    console.log(`   - Duplicates removed: ${duplicates.length}`);
    
    if (fixes.length > 0) {
      console.log('\n📝 [DRAMA COLLECTION CLEANUP] Fixes applied:');
      fixes.forEach(fix => console.log(`   - ${fix}`));
    }
    
    if (duplicates.length > 0) {
      console.log('\n🔄 [DRAMA COLLECTION CLEANUP] Duplicates removed:');
      duplicates.forEach(dup => console.log(`   - ${dup}`));
    }
    
    // Write the updated data back to the file
    console.log('💾 [DRAMA COLLECTION CLEANUP] Writing updated data to file...');
    fs.writeFileSync(collectionsPath, JSON.stringify(collectionsData, null, 2), 'utf8');
    
    console.log('🎉 [DRAMA COLLECTION CLEANUP] Successfully cleaned Drama collection!');
    console.log(`📁 [DRAMA COLLECTION CLEANUP] Updated file: ${collectionsPath}`);
    
  } else {
    console.log('❌ [DRAMA COLLECTION CLEANUP] Drama collection not found in collections data');
  }
  
} catch (error) {
  console.error('❌ [DRAMA COLLECTION CLEANUP] Error cleaning Drama collection:', error);
}
