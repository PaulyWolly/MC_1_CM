// Fix favorites categorization - FINAL VERSION - BROWSER VERSION
// Run this in the browser console (F12 → Console tab)

console.log('🔧 Fixing favorites categorization - FINAL VERSION...');
...
if (stored) {
  try {
    ...
    if (window.mediaLibraryManager && window.mediaLibraryManager.unifiedData) {
      ...
      // Process movies array - move TV shows to TV shows array
      if (favorites.movies && Array.isArray(favorites.movies)) {
        favorites.movies.forEach(item => {
          ...
          // Use unified data type field as the source of truth
          if (itemData && itemData.type === 'tv-show') {
            // This is a TV show - move to TV shows array
            correctedTVShows.push(item);
            console.log('📺 Moved TV show from movies to TV shows array:', path, 'unified type:', itemData.type);
          } else if (itemData && itemData.type === 'movie') {
            // This is a movie - keep in movies array
            correctedMovies.push(item);
            console.log('🎬 Kept movie in movies array:', path, 'unified type:', itemData.type);
          } else {
      ...
      // Process TV shows array - move movies to movies array
      if (favorites.tvshows && Array.isArray(favorites.tvshows)) {
        favorites.tvshows.forEach(item => {
          ...
          if (itemData && itemData.type === 'movie') {
            ...
            correctedMovies.push(item);
            console.log('🎬 Moved movie from TV shows to movies array:', path, 'unified type:', itemData.type);
          } else if (itemData && itemData.type === 'tv-show') {
            // This is a TV show - keep in TV shows array
            correctedTVShows.push(item);
            console.log('📺 Kept TV show in TV shows array:', path, 'unified type:', itemData.type);
          } else {
            // Fallback: check for seasons property (legacy detection)
            if (itemData && itemData.seasons && typeof itemData.seasons === 'object') {
              correctedTVShows.push(item);
      ...
      // Show some examples of what was moved
      if (correctedTVShows.length > 0) {
        console.log('📺 TV Shows that were moved/corrected:');
        correctedTVShows.slice(0, 5).forEach(item => {
          console.log('  -', item.name || item.title || item.normalizedKey);
        });
      }
...
console.log('4. Future heart clicks will use the unified data type field!');
