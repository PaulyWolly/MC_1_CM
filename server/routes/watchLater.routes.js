...
// DELETE - Remove item from watch later by path
router.delete('/remove-by-path', async (req, res) => {
    try {
        const { path, title, type, mediaType } = req.body;
        
        if (!path && !title) {
            return res.status(400).json({ 
                error: 'Missing required fields: path or title' 
            });
        }
        
        console.log('[WATCH-LATER-API] Removing item by path:', path, 'title:', title, 'type:', type);
        
        const collection = await WatchLater.getDefaultCollection();
        const originalCount = collection.items.length;
        
        // Remove items that match the path or title
        collection.items = collection.items.filter(item => {
            if (path) {
                // Check if any of the item's path properties match
                const itemPaths = [
                    item.path,
                    item.relPath,
                    item.filePath,
                ...
                const pathMatch = itemPaths.some(itemPath => 
                    itemPath.replace(/\\/g, '/').toLowerCase().includes(path.replace(/\\/g, '/').toLowerCase())
                );
                
                if (pathMatch) {
                    console.log('[WATCH-LATER-API] Found path match:', item.title);
                    return false; // Remove this item
                }
            }
            
            if (title && item.title && item.title.toLowerCase().includes(title.toLowerCase())) {
                console.log('[WATCH-LATER-API] Found title match:', item.title);
                return false; // Remove this item
            }
        ...
        const removedCount = originalCount - collection.items.length;
        
        if (removedCount > 0) {
            await collection.save();
            console.log('[WATCH-LATER-API] Removed', removedCount, 'items by path/title. Total items:', collection.itemCount);
            
            res.json({
                success: true,
                message: `${removedCount} item(s) removed from watch later`,
                itemCount: collection.itemCount,
                removedCount: removedCount
            });
        } else {
            console.log('[WATCH-LATER-API] No items found to remove');
            res.json({
                success: true,
        ...
        console.error('[WATCH-LATER-API] Remove by path error:', error);
        res.status(500).json({ error: 'Failed to remove item by path from watch later' });
    }
});

// DELETE - Remove item from watch later by title
router.delete('/remove-by-title', async (req, res) => {
    try {
        const { title, type, mediaType } = req.body;
        ...
        console.log('[WATCH-LATER-API] Removing item by title:', title, 'type:', type);
        
        const collection = await WatchLater.getDefaultCollection();
        const originalCount = collection.items.length;
        
        // Remove items that match the title
        collection.items = collection.items.filter(item => {
        ...
        if (removedCount > 0) {
            await collection.save();
            console.log('[WATCH-LATER-API] Removed', removedCount, 'items by title. Total items:', collection.itemCount);
        ...
        console.error('[WATCH-LATER-API] Remove by title error:', error);
        res.status(500).json({ error: 'Failed to remove item by title from watch later' });
    }
});
