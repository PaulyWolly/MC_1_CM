...
// Create admin user endpoint (protected, requires superadmin or CLI access)
app.post('/api/auth/create-admin', async (req, res) => {
    try {
        ...
        if (existingUser) {
            ...
            console.log(chalk.green(`[AUTH] Existing user updated to admin: ${email}`));
        ...
        console.log(chalk.green(`[AUTH] New admin user created: ${email}`));
        ...
        console.error(chalk.red('[AUTH] Create admin error:'), error);
        res.status(500).json({ 
            success: false, 
            message: 'Internal server error' 
        });
...
app.post('/api/tts', async (req, res) => {
    ...
    console.log('- SPEECH_REGION present:', !!process.env.SPEECH_REGION);
    console.log('- SPEECH_REGION value:', process.env.SPEECH_REGION);
