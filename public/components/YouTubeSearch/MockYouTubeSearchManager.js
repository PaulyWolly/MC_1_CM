...
export default class YouTubeSearchManager {
    ...
    renderRealYoutubeResults(videos, page, totalPages, subject, type = 'search', isPagination = false) {
        ...
        // Humanize the query for display
        const cleanSubject = subject.replace(/^youtube\s+(search|play)\s+/i, '');
        const humanizedSubject = this.humanizeQuery ? this.humanizeQuery(cleanSubject) : cleanSubject.replace(/[._]/g, ' ').replace(/\b\w/g, l => l.toUpperCase());
        let headerTitle = `📺 <span style="color: #0066cc !important; font-weight: bold !important;">YouTube Results:</span> <span style="color: #000000 !important; font-weight: bold !important;">"${humanizedSubject}"</span>`;
