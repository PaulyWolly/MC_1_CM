...
class MediaLibraryManager {
  ...
  async loadUnifiedData() {
    ...
    try {
      // Load both movies and TV shows unified data
      const [moviesResponse, tvShowsResponse] = await Promise.all([
        fetch('/components/MediaLibrary/data/movies/movies-unified.json?t=' + Date.now()),
        fetch('/components/MediaLibrary/data/tv-shows/tv-shows-unified.json?t=' + Date.now())
      ]);
      ...
      if (moviesResponse.ok && tvShowsResponse.ok) {
        ...
        // Merge both datasets into unifiedData
        this.unifiedData = { ...moviesData, ...tvShowsData };
        
        console.log("[FORCE-NEW-DATA] ✅ Movies unified data loaded:", Object.keys(moviesData).length, "movies");
        console.log("[FORCE-NEW-DATA] ✅ TV Shows unified data loaded:", Object.keys(tvShowsData).length, "TV shows");
        console.log("[FORCE-NEW-DATA] ✅ Total unified data loaded:", Object.keys(this.unifiedData).length, "items");
      } else {
        throw new Error(`Failed to load unified data: Movies ${moviesResponse.status}, TV Shows ${tvShowsResponse.status}`);
      }
    } catch (error) {
  ...
  async syncWatchLaterToMongoDB() {
    try {
      ...
      for (const item of watchLaterData) {
        try {
          ...
          if (response.ok) {
            ...
            console.error(`❌ [SYNC-TO-MONGODB] Failed to sync: ${item.title} - ${response.status}`);
            errorCount++;
          }
        } catch (error) {
          console.error(`❌ [SYNC-TO-MONGODB] Error syncing ${item.title}:`, error);
          errorCount++;
        }
      }
      ...
      console.log(`📊 [SYNC-TO-MONGODB] Sync completed: ${successCount} success, ${errorCount} errors`);
      
    } catch (error) {
      console.error('❌ [SYNC-TO-MONGODB] Error:', error);
      this.showToast('Error syncing to MongoDB: ' + error.message, 'error');
    } finally {
  ...
  async getStructuredCollections() {
    ...
    try {
      ...
      if (collectionsData) {
        ...
        console.log('[DEBUG - COLLECTIONS] Loaded structured collections from localStorage:', Object.keys(collections).length, 'collections');
        return this.structuredCollectionsData;
      }
    } catch (error) {
    ...
    this.structuredCollectionsData = { collections: { my_collections: {} } };
    return this.structuredCollectionsData;
  }
  /**
  ...
  async saveCollections(collections) {
    ...
    try {
      ...
      console.log('[DEBUG - COLLECTIONS] Collections saved to localStorage successfully');
    } catch (error) {
  ...
  async getItemCollections(path) {
    try {
      ...
      let unifiedKey = null;
      if (this.unifiedData) {
        console.log('[DEBUG - COLLECTIONS] Searching unified data for path:', path);
        ...
        if (!unifiedKey) {
          console.log('[DEBUG - COLLECTIONS] No unified key found for path:', path);
        }
      } else {
      ...
      const normalizedKey = this.normalizePath(path).replace(/[^a-z0-9.]/g, '.').replace(/\.+/g, '.').replace(/^\.|\.$/g, '');
      ...
      for (const [collectionName, paths] of Object.entries(collections)) {
        if (paths && paths.length > 0) {
          ...
          const pathMatch = paths.some(
            (storedPath) => {
              if (typeof storedPath === 'string') {
                return this.normalizePath(storedPath) === normalizedPath;
              }
              return false;
            }
          );
          ...
          const keyMatch = paths.some(
            (storedPath) => {
              if (typeof storedPath === 'string') {
                return this.normalizePath(storedPath) === normalizedKey;
              }
              return false;
            }
          );
          ...
          const unifiedMatch = unifiedKey && paths.some(
            (storedPath) => storedPath === unifiedKey
          );
  ...
  async showAddToCollectionModal(mediaItem) {
    try {
      ...
      // Add the collection types to the collectionsData structure for the modal
                                    ...
                                    ${(() => {
                                      ...
                                      for (const category of categoryOrder) {
                                        ...
                                        if (collections) {
                                          ...
                                          // Check if this category has any collections (now arrays instead of objects)
                                          if (collections && Array.isArray(collections) && collections.length > 0) {
                                            // Sort collections alphabetically
                                            const sortedCollections = collections.sort((a, b) => a.localeCompare(b));
                                            
                                            html += sortedCollections.map(collectionName => {
                                              ...
                                              return `
                                             <label class="collections-modal-checkbox-item">
                                                  <input type="checkbox" value="${collectionName}" ${isInCollection ? "checked disabled" : ""}>
      ...
      modal.addEventListener("click", (e) => {
       ...
       const addNewItemToCategory = async (input, button, category) => {
         ...
         // PERSISTENCE: Save the new item to collections.json and update the actual collections data
         try {
           console.log(`[DEBUG - COLLECTIONS] Persisting new ${category.toLowerCase().slice(0, -1)}: ${newItemName}`);
           
           // 1. Add the new collection to collections.json in the appropriate category
           const collectionsData = await this.getStructuredCollections();
           
           // Ensure the category exists
           if (!collectionsData.collections[category]) {
             collectionsData.collections[category] = {};
           }
           
           // Add the new collection (empty for now, will be populated when items are added)
           collectionsData.collections[category][newItemName] = [];
           
           // Save to collections.json via API
           const saveResponse = await fetch('/api/collections/save-json', {
             method: 'POST',
             headers: { 'Content-Type': 'application/json' },
           ...
           if (saveResponse.ok) {
             console.log(`[DEBUG - COLLECTIONS] Successfully added new collection "${newItemName}" to ${category} in collections.json`);
           } else {
             console.warn(`[DEBUG - COLLECTIONS] Failed to update collections.json:`, saveResponse.status);
           }
           
           // 2. Update the collectionsData in memory for this modal session
           if (!collectionsData.collections[category]) {
             collectionsData.collections[category] = {};
           }
           collectionsData.collections[category][newItemName] = [];
      ...
      addBtn.addEventListener("click", async (e) => {
        ...
        try {
          // Get current collections to determine what to add/remove
          ...
          console.log('[DEBUG - COLLECTIONS] Collections to remove:', collectionsToRemove);
          ...
          if (collectionsToRemove.length > 0) {
            const removeResult = await this.removeFromCollection(
              collectionsToRemove,
              mediaItem.path
            );
            if (!removeResult.success) {
              this.showToast(
                removeResult.message || "Error removing from collections",
                "error"
              );
              return;
            }
          ...
          // Show success message
          const totalChanges = collectionsToAdd.length + collectionsToRemove.length;
          if (totalChanges > 0) {
              const mediaTitle = this.getDisplayTitle(mediaItem);
              this.showToast(
  ...
  getSeasonsForShow(showOrPath) {
    ...
    if (this.unifiedData && showName) {
      ...
      // Convert showName to normalized key format (same logic as findShowByPath)
                    ...
                    <div class="${gridClass}">
                        ${seasons
                          .map((season) => {
                            ...
                            let seasonImage = season.poster || season.image || '/assets/img/placeholder-poster.jpg';
