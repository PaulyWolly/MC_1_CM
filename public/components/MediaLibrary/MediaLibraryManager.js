            ...
            document.querySelectorAll('.watch-later-delete-btn').forEach(btn => {
                btn.onclick = async (e) => {
                    ...
                    if (card) {
                        ...
                        console.log('[WATCH-LATER-DELETE] Movie path from data-path:', path);
                        
                        // For TV shows, extract path from episode data
                        if (!path) {
                            const episodeData = card.getAttribute('data-episode');
                            console.log('[WATCH-LATER-DELETE] Episode data found:', !!episodeData);
                            if (episodeData) {
                                try {
                                    const episodeObj = JSON.parse(episodeData);
                                    path = episodeObj.path;
                                    console.log('[WATCH-LATER-DELETE] TV show path extracted:', path);
                                } catch (error) {
                    ...
                    if (path) {
                        console.log('[WATCH-LATER-DELETE] Proceeding with deletion of:', path);
                        console.log('[WATCH-LATER-DELETE] Path type:', typeof path);
                        console.log('[WATCH-LATER-DELETE] Path length:', path.length);
                        console.log('[WATCH-LATER-DELETE] Path contains TV-SHOWS:', path.includes('TV-SHOWS'));
                        
                        // Debug: Show current localStorage content before deletion
                        const currentResumeList = JSON.parse(localStorage.getItem('mediaLibraryResumeList') || '[]');
                        console.log('[WATCH-LATER-DELETE] Current localStorage content:', currentResumeList.map(item => ({
                            title: item.title,
                            path: item.path,
                            type: item.type,
...
=======
    ...
    isInCollectionSync(path) {
        try {
            ...
            console.error('[DEBUG - COLLECTIONS] Error checking collection membership (sync):', error);
            return false;
        }
    }
        ...
        try {
            ...
            Object.entries(collections).forEach(([collectionName, paths]) => {
                if (paths && Array.isArray(paths)) {
                    const hasMovie = paths.some(storedPath => 
                        this.normalizePath(storedPath) === normalizedPath
                    );
                    if (hasMovie) {
            ...
            console.error('[DEBUG - COLLECTIONS] Error getting collection name for movie:', error);
            return 'Unknown Collection';
        }
    }
...
>>>>>>> local
    ...
    async updateMovieDetailsCollectionInfo(movie) {
        try {
            ...
            const collectionInfo = document.querySelector('.media-library-details-collection-info');
            ...
            collectionBtn.title = inCollection ? 'Remove from Collection' : 'Add to Collection';
            ...
            if (inCollection) {
                ...
                if (collectionInfo) {
                    ...
                    const buttonsDiv = document.querySelector('.media-library-details-buttons');
                    if (buttonsDiv && buttonsDiv.nextSibling) {
                        buttonsDiv.parentNode.insertBefore(newCollectionInfo, buttonsDiv.nextSibling);
                    }
                }
            } else {
            ...
            console.log('[DEBUG - MOVIE-DETAILS] Collection info updated for:', movie.path, 'In collection:', inCollection);
        } catch (error) {
            console.error('[DEBUG - MOVIE-DETAILS] Error updating collection info:', error);
        }
    }
    ...
    getEpisodesForSeason(showPath, seasonPath) {
        ...
        if (show.folders && Array.isArray(show.folders) && show.folders.length > 0) {
            ...
            const seasonFolder = show.folders.find(folder => {
                ...
                return normalizedFolderPath.includes(normalizedSeasonPath) || 
        ...
        if (show.seasons && Array.isArray(show.seasons) && show.seasons.length > 0) {
            ...
            if (show.seasons[0] && typeof show.seasons[0] === 'object' && show.seasons[0].episodes) {
                ...
                // Find season by name (e.g., "Season 01")
                const season = show.seasons.find(s => s.name === seasonPath || s.path === seasonPath);
                if (!season) {
                    console.log('[EPISODE DEBUG] Season not found:', seasonPath);
                    return [];
                }
        ...
        if (removedCount === 0) {
            ...
            if (titleToRemove) {
                ...
                resumeList = resumeList.filter(item => {
                    ...
                    const itemFilename = (item.path || '').split(/[\\/]/).pop()?.replace(/\.[^/.]+$/, '').toLowerCase();
                    
                    const titleMatch = itemTitle.includes(titleToRemove.toLowerCase()) || 
            ...
            if (removedCount === 0) {
                ...
                resumeList = resumeList.filter(item => {
                    ...
                    const itemShowName = (item.title || '').split(':')[0]?.toLowerCase() || '';
                    const itemEpisode = (item.title || '').split(':')[1]?.toLowerCase() || '';
                    
                    const showMatch = showName.toLowerCase().includes(itemShowName) || 
                                    itemShowName.includes(showName.toLowerCase());
                    const episodeMatch = episodeFile.toLowerCase().includes(itemEpisode) || 
