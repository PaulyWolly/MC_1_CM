...
class MediaLibraryManager {
  ...
  getCollectionsAsArray() {
    try {
      ...
      const allCollectionNames = Object.keys(collections).filter(name => name !== 'My PICK');
      ...
      console.error('[COLLECTIONS-ARRAY] Error getting collections as array:', error);
      return [];
    }
  }
  ...
  saveCollectionsArray(collectionsArray) {
    try {
      ...
      const order = collectionsArray.map(c => c.name).filter(name => name !== 'My PICK');
      this.saveCollectionOrder(order);
      
      console.log('[COLLECTIONS-ARRAY] Saved collections array:', collectionsArray.map(c => c.name));
    } catch (error) {
      console.error('[COLLECTIONS-ARRAY] Error saving collections array:', error);
    }
  }
  ...
  async reorderCollections(draggedCollection, targetCollection) {
    try {
      console.log('🎯 [COLLECTION-ARRAY] Starting reorder:', { draggedCollection, targetCollection });
      ...
      const collectionsArray = this.getCollectionsAsArray();
      console.log('🎯 [COLLECTION-ARRAY] Current collections array:', collectionsArray.map(c => c.name));
      
      // Find indices
      const draggedIndex = collectionsArray.findIndex(c => c.name === draggedCollection);
      const targetIndex = collectionsArray.findIndex(c => c.name === targetCollection);
      ...
      console.log('🎯 [COLLECTION-ARRAY] After reorder:', reorderedArray.map(c => c.name));
      
      // Save the new array order
      this.saveCollectionsArray(reorderedArray);
      console.log('🎯 [COLLECTION-ARRAY] Saved new array order');
      
      // Re-render the collections tab
      console.log('🎯 [COLLECTION-ARRAY] Re-rendering collections tab...');
      ...
      if (mediaGrid) {
        ...
        console.log('🎯 [COLLECTION-ARRAY] Updated mediaGrid with new HTML');
        
        // Re-attach click handlers after DOM update
        setTimeout(() => {
          console.log('🎯 [COLLECTION-ARRAY] Re-attaching click handlers...');
          this.attachCollectionClickToMove();
          console.log('🎯 [COLLECTION-ARRAY] Click handlers re-attached');
          
          // Verify handlers are working
          const collectionCards = document.querySelectorAll('.media-library-movie-card-movies-collections');
          console.log('🎯 [COLLECTION-ARRAY] Found', collectionCards.length, 'collection cards after re-attachment');
          ...
          console.log('🎯 [COLLECTION-ARRAY] Current DOM order:', cardNames);
        }, 100);
      } else {
      ...
      this.showToast(`Moved "${draggedCollection}" to position of "${targetCollection}"`, 'success');
