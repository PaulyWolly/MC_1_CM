// Fix favorites categorization - CORRECTED BROWSER VERSION
// Run this in the browser console (F12 → Console tab)

console.log('🔧 Fixing favorites categorization (corrected version)...');
    ...
    if (window.mediaLibraryManager && window.mediaLibraryManager.unifiedData) {
      ...
      if (favorites.movies && Array.isArray(favorites.movies)) {
        favorites.movies.forEach(item => {
          ...
          // Check multiple type indicators
          const isTVShow = (itemData && (itemData.type === 'tv-show' || itemData.type === 'tv')) ||
                          (item && (item.type === 'tv-show' || item.type === 'tv')) ||
                          (itemData && itemData.seasons && typeof itemData.seasons === 'object');
          
          if (isTVShow) {
            // This is a TV show - move to TV shows array
            correctedTVShows.push(item);
            console.log('📺 Moved TV show from movies to TV shows array:', path, 'type:', item.type || itemData?.type);
          } else {
            // This is a movie - keep in movies array
            correctedMovies.push(item);
            console.log('🎬 Kept movie in movies array:', path, 'type:', item.type || itemData?.type);
          }
        });
      }
      ...
      if (favorites.tvshows && Array.isArray(favorites.tvshows)) {
        favorites.tvshows.forEach(item => {
          ...
          // Check multiple type indicators
          const isMovie = (itemData && itemData.type === 'movie') ||
          ...
          if (isMovie) {
            ...
            correctedMovies.push(item);
            console.log('🎬 Moved movie from TV shows to movies array:', path);
          } else {
            // This is a TV show - keep in TV shows array
            correctedTVShows.push(item);
            console.log('📺 Kept TV show in TV shows array:', path);
          }
        });
      }
